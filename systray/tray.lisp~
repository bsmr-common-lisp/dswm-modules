;;;; System Tray for dswm. 

;;;; Note: To run this you need to patch dswm to add
;;;; dswm:*event-processing-hook*
...
(defstruct (tray)
  ...
  event-processing-fn ;; Hook function added to dswm:*event-processing-hook* to process tray events
  display
  screen)
...
(defun screen-mode-line (screen)
  ...
  (dswm::head-mode-line 
   (funcall *tray-head-selection-fn* 
	    (dswm::screen-heads screen))))
(defun current-tray ()
  (screen-tray (dswm:current-screen)))

;;;; Tray appearance
;;; Dimensions The tray height depends on the modeline height. In
;;; dswm there is a modeline for every X screen.
...
SCREEN's modeline."
  (xlib:drawable-height
   (dswm::mode-line-window (screen-mode-line screen))))
...
(defun create-tray (screen parent &key (x 0) (y 0))
  "Creates a tray object given the SCREEN and the PARENT window. The
  tray object needs further initialization, see `tray-init'."
  (let* ((root-window (xlib:drawable-root parent))
...
(defun destroy-tray (tray)
  ...
  (dswm:remove-hook dswm:*event-processing-hook* (tray-event-processing-fn tray))
...
System Tray protocol."
  (let* ((dswm-screen (tray-screen tray))
	 (screen (slot-value dswm-screen 'dswm::number))
...
(dswm:defcommand dstray () ()
  (unless (current-tray)
    (let* ((dswm-screen (dswm:current-screen))
	   (mlw (dswm::mode-line-window
		 (screen-mode-line dswm-screen)))
	   (tray (create-tray dswm-screen mlw))
	   (hnd (make-tray-handler tray)))
      (setf (screen-tray dswm-screen) tray)
      (tray-init tray)
      (map-tray tray)
      (let ((event-handler (lambda ()
			     (loop while (ignore-errors
					   (xlib:process-event dswm::*display* :timeout 0 :handler hnd))))))
	(setf (tray-event-processing-fn tray) event-handler)
	(dswm::add-hook dswm:*event-processing-hook*
			   event-handler)))))


(dswm:defcommand dstray-toggle-hidden-icons-visibility () ()
...
(dswm:defcommand systray-selection-right () ()
...
(dswm:defcommand systray-selection-left () ()
  (let* ((tray (current-tray))
	 (pos (tray-curpos tray)))
    (show-hiwin tray)
...
(dswm:defcommand systray-toggle-icon-hiding () ()
  (let ((tray (current-tray)))
    (toggle-icon-hiding tray (icon-at-cursor tray))
    (tray-update tray)))

(dswm:defcommand systray-move-icon-left () ()
  (let ((tray (current-tray)))
    (show-hiwin tray)
    (move-icon-left tray)
    (tray-update tray)))

(dswm:defcommand systray-move-icon-right () ()
  (let ((tray (current-tray)))
    (show-hiwin tray)

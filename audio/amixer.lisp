...
(defvar *amixer-program* (get-audio-backend-command "amixer"))

(defun volcontrol (channel amount)
  (let ((percent (parse-integer
		  (run-shell-command
		   (concat *amixer-program* " sset " channel " " (or amount "toggle")
			   "| tail -1"
			   "| sed 's/^.*\\[\\([[:digit:]]\\+\\)%\\].*$/\\1/'")
		   t))))
    (message
...
(defcommand amixer-sense-toggle () ()
  (message
   (concat "Headphone Jack Sense toggled"
           (run-shell-command (concat *amixer-program* " sset 'Headphone Jack Sense' toggle") t))))
